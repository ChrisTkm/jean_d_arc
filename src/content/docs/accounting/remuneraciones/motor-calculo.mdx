...
<Aside type="tip" title="Filosofía">
  Esta arquitectura separa claramente responsabilidades: - **PostgreSQL**:
  Persistencia y lecturas eficientes (Smart Views). - **Orchestrator
  (Node.js)**: Lógica de negocio compleja, validaciones y cálculo.
</Aside>
...
<Aside type="caution" title="Pureza Funcional">
  **PayrollEngine** es una función pura estática. No realiza I/O. - **Entrada**:
  `PayrollInput` (Interfaz estricta). - **Salida**: `PayrollResult` (Resultado
  matemático).
</Aside>
...
Centraliza el SQL. Reemplaza la necesidad de tener lógica de negocio dispersa en múltiples queries. Utiliza `Promise.all` para paralelizar la obtención de Contrato, Asistencia e Indicadores.
